...
def createParts(nodes, locs, nParts, nodeConnections, locations, maxDist):
...
def matchParts(nodesInPart, locNodesInPart, nParts, nodeConnections, locations, distances, maxDist, leftoverLocs, leftoverNodes, nodesToLoc):
    ...
    minTotalDistance = maxDist*maxDist*(len(locations)**2)*2
    minCombination = []
    ...
    for i in minCombination:
        ...
        if newNParts > 1 and locParts > 1:
            ...
            nNodesInPart, nLocNodesinPart  = createParts(nodesInPart[i[0]], nLocations, newNParts, nodeConnections, locations, maxDist)
            leftoverLocs, leftoverNodes, nodesToLoc = matchParts(nNodesInPart, nLocNodesinPart, newNParts, nodeConnections, locations, distances, maxDist, leftoverLocs, leftoverNodes)
        else:
            for idx, node in enumerate(nodesInPart[i[0]]):
                ...
                nodesToLoc = matchNodesinPart(nodesToMatch, locsToMatch, nodeConnections, nodesToLoc, distances, maxDist)
...
def matchNodesinPart(nodes, locs, nodeConnections, nodesToLoc, distances, maxDist):
    ...
    while nodes:
        ...
        for n in nodeConnections:
            if n in nodes:
                closeLoc = closestLocation(currentLoc, locs, maxDist)
                nodesToLoc[n] = closeLoc
                nodes.remove(n)
                locs.remove(closeLoc)
    return nodesToLoc

def metisLeftover(leftoverLocs, leftoverNodes, nodeConnections, locations, maxDist, nodesToLoc):
...
def clustering(matrix, locations, knownLocations):
    # Input:
    ...
    nodesInPart, locNodesInPart = createParts(nodes, locs, parts, nodeConnections, locations, maxDist)
    ...
    leftoverLocs, leftoverNodes, nodesToLoc = matchParts(nodesInPart, locNodesInPart, parts, nodeConnections, locations, distances, maxDist, leftoverLocs, leftoverNodes, nodesToLoc)
    leftoverLocs, nodesToLoc = metisLeftover(leftoverLocs, leftoverNodes, nodeConnections, locations, maxDist, nodesToLoc)
